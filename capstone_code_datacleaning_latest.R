...
#Correcting column value in "area_name".
...
#Adding Hawaii at the correct position and removing Panama which
# is not present in other data sets.
...
# primary_trump_todate does not have candidate, fips, party variables.
...
# Doing the entire thing for Hillary Clinton.
primary_results_complete_hillary <- filter(primary_results._complete, candidate == "Hillary Clinton")
primary_hillary <- select(primary_results_complete_hillary, -c(candidate, fips, county, state_abbreviation, party))
primary_hillary_1 <- primary_hillary %>%
  + group_by(state) %>%
  + summarise_each(funs(sum))
primary_hillary_2 <- mutate(primary_hillary_1, frac_vote = round(fraction_votes))
primary_hillary_3 <- select(primary_hillary_2, -fraction_votes)
primary_hillary_4 <- filter(primary_hillary_3, state != "Colorado")
primary_hillary_5 <- filter(primary_hillary_4, state != "Colorado")
primary_hillary_6 <- filter(primary_hillary_5, state != "North Dakota")
primary_hillaryC <- select(primary_hillary_6, -frac_vote)
colnames(primary_hillaryC)[2] <- "votesH"
primary_clinton <- select(primary_hillaryC, -state)
full_data_clinton <- bind_cols(full_data_15, primary_clinton)
full_data_clinton_1 <- mutate(full_data_clinton, absolute_votes_clinton = round(as.numeric(paste(votesH))/as.numeric(paste(voting_population))*100))

data_economy_new_clinton <- bind_cols(data_economy_new, primary_clinton)
clinton_economy_tree = rpart(votesH, Farming + Mining + Manufacturing + Government + Recreation + Nonspecialized, data = data_economy_new_clinton)

# Vote percentage for Clinton.
clinton_votep <- select(full_data_clinton_1, absolute_votes_clinton)
colnames(clinton_votep)[1] <- "VotesC"

# Economy vs Clinton
data_economyClinton <- bind_cols(data_economy_new, clinton_votep)
clinton_economy_tree = rpart(VotesC ~ Farming + Mining + Manufacturing + Government + Recreation + Nonspecialized, data = data_economyClinton)
fancyRpartPlot(clinton_economy_tree)

# Race vs Clinton
data_raceClinton <- bind_cols(data_race1, clinton_votep)
clinton_race_tree = rpart(VotesC ~ White + Black + Native + Asian + Islander + Mixed + Hispanic, data = data_raceClinton)
fancyRpartPlot(clinton_race_tree)
write.csv(data_raceClinton, file = "race_clinton.csv")

# Major races vs Clinton
clinton_racemajor_tree = rpart(VotesC ~ White + Black + Hispanic, data = data_raceClinton)
fancyRpartPlot(clinton_racemajor_tree)

# Linear regression for Clinton with Whites.
clinton_white_lm = lm(VotesC ~ White, data = data_raceClinton)
summary(clinton_white_lm)
